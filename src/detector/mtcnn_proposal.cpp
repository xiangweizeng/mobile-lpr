...
namespace pr
{
    MtcnnProposal::MtcnnProposal(std::string param, std::string bin, int width, int height, float iou,
                                 float threshold) : MtcnnBase(param, bin, width, height, iou, threshold) {
        min_size = 80;
        factor = 0.709f;
        pnet_stride = 2;
