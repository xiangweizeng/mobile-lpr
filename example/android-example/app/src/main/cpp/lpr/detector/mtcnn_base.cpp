...
namespace pr
{
    ...
    MtcnnBase::MtcnnBase(AAssetManager* mgr, std::string param, std::string bin, int width, int height, float iou, float threshold) {
        this->width = width;
        this->height = height;
        this->iou_threhold = iou;
        this->prob_threhold = threshold;

        if(0 != net.load_param(mgr, param.c_str())){
            printf("Load param failed for mtcnnbase\n");
            return ;
        }

        if(0 != net.load_model(mgr, bin.c_str())){
